...
class Snake(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.statusBar().showMessage("Итоговый счет = %d" % self.score)
    ...
    def __init_plots(self):
        ...
        # скрыть оси и все кнопки, создать визуальное поле
    ...
    def failureMsgBox(self):
        ...
        Mbox.setText("Вы проиграли")
        ExitButton = Mbox.addButton('Выйти', QMessageBox.AcceptRole)
        Mbox.addButton('Перезапуск', QMessageBox.RejectRole)
        Mbox.setModal(True)
        Mbox.exec()
    ...
    def buttonPauseClicked(self):
        ...
        Mbox.setText("Пауза")
        quitButton = Mbox.addButton('Продолжить', QMessageBox.AcceptRole)
        Mbox.exec()
...
if __name__ == '__main__':
    ...
    window = Snake()
    window.setWindowTitle("Змейка Симонов")
    window.show()
    sys.exit(app.exec_())